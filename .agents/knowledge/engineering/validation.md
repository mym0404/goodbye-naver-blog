...
- `pnpm parser:check`: parser block registry, parser fixture, sample fixture, 테스트/sample 연결과 generated knowledge projection freshness가 구조적으로 맞는지 확인할 때 실행한다. `parser-fixture` 분류 자체의 타당성은 자동 판정하지 않는다.
...
- `pnpm quality:report`: parser/sample knowledge projection과 parser block fixture coverage, parser block test mapping coverage, parser block/sample coverage generated 품질 리포트를 다시 만들 때 실행한다.
...
- `pnpm samples:verify`는 `parser-fixture` parser block를 보장하지 않는다. 이 범위는 parser unit test와 parser fixture가 맡는다.
...
- parser block/sample/harness 변경 뒤에는 `pnpm typecheck`, `pnpm test:offline`, `pnpm parser:check`, `pnpm samples:verify`, `pnpm quality:report`를 우선 본다.
- parser block registry를 바꿀 때는 `sample-fixture`와 `parser-fixture` 분류를 함께 검토한다. 공개 글을 끝내 확보하지 못한 parser block를 억지로 sample gap으로 남기지 않는다.
