...
## 목적
이 문서는 parser block, sample fixture, UI smoke, generated 리포트가 어떤 역할로 맞물리는지 정리한다.

## Source Of Truth
- 실제 harness 동작은 `package.json`, `scripts/harness/*`, generated 보고서 출력 경로가 기준이다.
- parser/sample knowledge projection의 canonical SoT는 `src/shared/BlockRegistry.ts`, `src/modules/blog/BlogRegistry.ts`, `src/shared/SampleCorpus.ts`다.
...
- `pnpm quality:report`: parser block/sample coverage generated 문서를 다시 만들 때 실행한다.

## Harness Roles
- `check-parser-blocks.ts`
  parser block id, parser fixture, sample fixture, 테스트, sample link 연결과 generated knowledge 문서 freshness가 끊기지 않았는지 확인한다.
...
- `generate-quality-report.ts`
  parser/sample knowledge projection과 parser block fixture coverage, parser block test mapping coverage, `sample-fixture` parser block coverage, `parser-fixture` parser block 목록을 generated markdown으로 다시 계산한다.
...
- 공개 글 fixture가 끝내 확보되지 않는 parser block는 `parser-fixture`로 분류한다. 이 경우 parser unit test와 parser fixture가 canonical 검증 경로다.
...
## Change Triggers
- parser block, sample corpus, sample fixture를 바꿨다면 `pnpm parser:check`, `pnpm samples:verify`, `pnpm quality:report`를 함께 본다.
...
- knowledge에서 generated 보고서를 인용할 때는 수치 자체보다 어떤 parser block gap이 비어 있는지 읽는 데 사용한다.
