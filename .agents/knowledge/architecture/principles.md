...
## 관련 코드
- `src/modules/exporter/NaverBlogExporter.ts`
...
- parser block는 구조 계약으로 취급하고, sample fixture는 `tests/fixtures/samples/*` 기준으로 관리한다.
- UI는 내부 모듈을 직접 읽지 않고 HTTP API만 호출한다.

## Change Implications
- parser 규약을 바꾸면 parser block 선언, sample fixture, parser 테스트가 같이 바뀌어야 한다.
