...
- `Status`: Done
- `Depends On`: None
- `Start When`: The bundle is approved and `src/modules/exporter/picgo-upload-phase.ts` still uploads all candidates in one batch without any intermediate callback.
- `Files`: primary: `src/modules/exporter/picgo-upload-phase.ts`, `tests/picgo-upload-phase.test.ts`; generated/incidental: N/A
...
- `Files`: primary: `src/server/http-server.ts`, `src/server/job-store.ts`, `src/modules/exporter/picgo-upload-rewriter.ts`, `tests/http-server.test.ts`; generated/incidental: N/A
...
- `Files`: primary: `src/modules/exporter/picgo-upload-phase.ts`, `src/server/http-server.ts`, `src/server/job-store.ts`, `src/modules/exporter/picgo-upload-rewriter.ts`, `src/ui/components/ui/progress.tsx`, `src/ui/features/job-results/job-results-panel.tsx`, `tests/picgo-upload-phase.test.ts`, `tests/http-server.test.ts`, `tests/ui/app.test.tsx`; generated/incidental: N/A
...
- `Files`: primary: `.agents/knowledge/product/domain.md`, `.agents/knowledge/product/ui-dashboard-design-system.md`, `.agents/knowledge/architecture/system-map.md`, `docs/runbooks/browser-verification.md`; generated/incidental: N/A
...
- `Implementation Notes`: `domain.md` should explain the product semantics, `ui-dashboard-design-system.md` should explain the screen contract and overflow rule, `system-map.md` should explain the state flow, and the runbook should explain how to verify partial progress in both mocked and live modes.
- `Verification Strategy`: Run `rg -n \"부분 완료|progress|uploadedCount|rewrite|max-height|test:network:upload\" .agents/knowledge/product/domain.md .agents/knowledge/product/ui-dashboard-design-system.md .agents/knowledge/architecture/system-map.md docs/runbooks/browser-verification.md`. Success signal is exit code `0` and the hits show the new contract in every intended document. If the command exits `0` but any document still describes only final upload completion or omits the overflow requirement, treat the task as incomplete and return the bundle for repair.
...
- `Files`: primary: `scripts/harness/run-ui-smoke.ts`, `scripts/harness/run-ui-live-upload.ts`, `.agents/knowledge/product/domain.md`, `.agents/knowledge/product/ui-dashboard-design-system.md`, `.agents/knowledge/architecture/system-map.md`, `docs/runbooks/browser-verification.md`; generated/incidental: smoke/live evidence artifacts
...
- `Verification Strategy`: 1. Run `pnpm smoke:ui` again. 2. If the live-upload env is present, rerun `pnpm test:network:upload`; otherwise verify the docs explicitly describe that live verification remains blocked. 3. Run `rg -n "부분 완료|rewrite|progress|uploading|max-height|master|FAREWELL_UPLOAD_E2E" .agents/knowledge/product/domain.md .agents/knowledge/product/ui-dashboard-design-system.md .agents/knowledge/architecture/system-map.md docs/runbooks/browser-verification.md`. Success signal is smoke evidence plus conditional live evidence aligning with the documented contract. If any command exits `0` but the bundle evidence still lacks browser proof for partial progress, full-bar rewrite-pending state, form visibility rules, `master` branch policy, or the docs remain inconsistent with the harness behavior, keep the task incomplete and return the bundle for repair.
...
- `Files`: primary: `src/modules/exporter/picgo-upload-phase.ts`, `src/modules/exporter/picgo-upload-rewriter.ts`, `src/server/http-server.ts`, `src/server/job-store.ts`, `src/ui/components/ui/progress.tsx`, `src/ui/features/job-results/job-results-panel.tsx`, `scripts/harness/run-ui-smoke.ts`, `scripts/harness/run-ui-live-upload.ts`, `.agents/knowledge/product/domain.md`, `.agents/knowledge/product/ui-dashboard-design-system.md`, `.agents/knowledge/architecture/system-map.md`, `docs/runbooks/browser-verification.md`, `tests/picgo-upload-phase.test.ts`, `tests/http-server.test.ts`, `tests/ui/app.test.tsx`; generated/incidental: coverage artifacts, smoke screenshots/logs, live upload evidence, any changed manifest fixture outputs
